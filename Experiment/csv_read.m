...
quant_1 = round(signal_2 * (2^8-1));

datas1 = floor(quant_1 / 2^6);
datas2 = floor(mod(quant_1, 2^6) / 2^4);
datas3 = floor(mod(quant_1, 2^4) / 2^2);
datas4 = mod(quant_1, 2^2);

datas_all = [datas1; datas2; datas3; datas4;];

%%
output = zeros(4,4,318*318);
ber = zeros(4,4,16,3);  % 光计算误码个数
BER = zeros(4,4);  % 输出切片误码个数
...
for i=1:4
    for j=1:4
        output_final = output_final + reshape(output(i,j,:)* 4 ^ (8 - i - j),318,318);
    end
end
...
output_quant = (output_final / (2^8-1)^2 * max(abs(weight), [], 'all') * (max(signal_1) - min(signal_1)) + sum(weight,"all") * min(signal_1));
